	...
	public void show() {
		...
		backButton.addListener(new InputListener() {
			...
			public void enter(InputEvent event, float x, float y, int pointer,
					Actor fromActor) {
				// Tween.from(backButton, ActorAccessor.X,
				// .75f).target(backButton.getX() - 10).start(tween);
				return;
			}
		});
		...
		// Tween.from(table, ActorAccessor.Y,
		// .75f).target(Gdx.graphics.getHeight() / 8).start(tween);

		// The Heading fade in
		Timeline.createSequence()
				.beginSequence()
				.push(Tween.from(heading, ActorAccessor.RGB, .75f).target(1, 0,
						1)).end().repeat(Tween.INFINITY, 0).start(tween);
	}
